...
export async function GET(request: NextRequest) {
  ...
  const result = await tryCatch(async () => {
    ...
    // ── Batch 1: All counts + groupBy + queries in parallel ──
    const [
      totalProperties,
      activeProperties,
      draftProperties,
      ...
      // 2 trend queries (lightweight — only select createdAt)
      db.lead.findMany({
    ...
    const leadsByStatus = leadsByStatusRaw.map((item: any) => ({
      status: item.status,
      count: item._count?.status || 0,
    }))

    // Process leads by source
    const leadsBySource = leadsBySourceRaw.map((item: any) => ({
      source: item.source,
      count: item._count?.source || 0,
    }))

    // Process leads per month
    ...
    while (monthIter <= now) {
      ...
      leadsPerMonth.push({ month: key, count: leadsPerMonthMap[key] || 0 })
      monthIter.setMonth(monthIter.getMonth() + 1)
    }

    // Process listings per month
    ...
    while (listingMonthIter <= now) {
      ...
      listingsPerMonth.push({ month: key, count: listingsPerMonthMap[key] || 0 })
